...
void drawScene() {
  ...
  if (first) {
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "Starting Asset Zoo with " << blockModels.size() << " models" << std::endl;
  ...
  for (int row = 0; row < gridSize; row++) {
    for (int col = 0; col < gridSize; col++) {
      if (index < blockModels.size()) {
        ...
        Renderer::renderColoredCube(x, -0.6f, z, 1.5f, 0.1f, 1.5f, r, g, b, 1.0f);
  ...
  // Draw a ground plane
  Renderer::renderColoredPlane(0.0f, -1.0f, 0.0f, 20.0f, 20.0f, 0.2f, 0.2f, 0.3f);
...
int main(int argc, char *argv[]) {
  ...
  while (running) {
    ...
    if (pad.Lx != 128 || pad.Ly != 128) {
      ...
      camera.posY -= sinf(pitchRad) * dy * MOVEMENT_SPEED; // Y movement based on pitch
