...
DisplayTransform Model::parseDisplayTransform(const nlohmann::json& transformJson) {
    ...
    if (transformJson.contains("translation")) {
        ...
        if (translation.is_array() && translation.size() == 3) {
            ...
            transform.translation = {
                ...
                std::max(-80.0f, std::min(80.0f, static_cast<float>(translation[2])))
            };
        }
    }
    ...
    if (transformJson.contains("scale")) {
        ...
        if (scale.is_array() && scale.size() == 3) {
            ...
            transform.scale = {
                std::min(4.0f, static_cast<float>(scale[0])),
...
ModelElement Model::parseElement(const nlohmann::json& elementJson) {
    ...
    if (elementJson.contains("to") && elementJson["to"].is_array() && elementJson["to"].size() == 3) {
        ...
        element.to = {
            std::max(-16.0f, std::min(32.0f, static_cast<float>(to[0]))),
    ...
    if (elementJson.contains("rotation")) {
        ...
        if (rotationJson.contains("origin") && rotationJson["origin"].is_array() && rotationJson["origin"].size() == 3) {
        ...
        if (rotationJson.contains("rescale") && rotationJson["rescale"].is_boolean()) {
    ...
    if (elementJson.contains("light_emission") && elementJson["light_emission"].is_number()) {
        element.light_emission = std::max(0, std::min(15, static_cast<int>(elementJson["light_emission"])));
    }
    ...
    if (elementJson.contains("faces") && elementJson["faces"].is_object()) {
        ...
        const std::array<std::string, 6> faceNames = {"down", "up", "north", "south", "west", "east"};
...
ModelFace Model::parseFace(const nlohmann::json& faceJson) {
    ...
    if (faceJson.contains("uv") && faceJson["uv"].is_array() && faceJson["uv"].size() == 4) {
        ...
        face.uv = UV(
            static_cast<float>(uv[0]) / 16.0f,
            static_cast<float>(uv[1]) / 16.0f,
            static_cast<float>(uv[2]) / 16.0f,
            static_cast<float>(uv[3]) / 16.0f
        );
    }
...
void Model::render() const {
    if (elements.empty()) {
        std::cerr << "Warning: Attempting to render model with no elements" << std::endl;
        return;
    }
    
    std::cout << "Rendering model with " << elements.size() << " elements" << std::endl;
...
void Model::renderFace(const std::string& faceName, const ModelFace& face, 
    ...
    // Originally we had to swap x1/x2 and y1/y2 to fix UV coordinate flipping
    float u1 = face.uv.x1;
    float v1 = face.uv.y2;
    float u2 = face.uv.x2;
    float v2 = face.uv.y1;
    ...
    if (faceName == "down") {
        // Down face (Y-)
        vertices[0] = x1; vertices[1] = y1; vertices[2] = z1;  // Bottom-left
        vertices[3] = x2; vertices[4] = y1; vertices[5] = z1;  // Bottom-right
        vertices[6] = x2; vertices[7] = y1; vertices[8] = z2;  // Top-right
        vertices[9] = x1; vertices[10] = y1; vertices[11] = z2; // Top-left
        
        texCoords[0] = u1; texCoords[1] = v1;
        texCoords[2] = u2; texCoords[3] = v1;
        texCoords[4] = u2; texCoords[5] = v2;
        texCoords[6] = u1; texCoords[7] = v2;
    }
    else if (faceName == "up") {
        // Up face (Y+)
        vertices[0] = x1; vertices[1] = y2; vertices[2] = z2;  // Bottom-left
        vertices[3] = x2; vertices[4] = y2; vertices[5] = z2;  // Bottom-right
        vertices[6] = x2; vertices[7] = y2; vertices[8] = z1;  // Top-right
        vertices[9] = x1; vertices[10] = y2; vertices[11] = z1; // Top-left
        ...
        // North face (Z-)
        vertices[0] = x2; vertices[1] = y1; vertices[2] = z1;  // Bottom-left
        vertices[3] = x1; vertices[4] = y1; vertices[5] = z1;  // Bottom-right
        vertices[6] = x1; vertices[7] = y2; vertices[8] = z1;  // Top-right
        vertices[9] = x2; vertices[10] = y2; vertices[11] = z1; // Top-left
        ...
        // South face (Z+)
        vertices[0] = x1; vertices[1] = y1; vertices[2] = z2;  // Bottom-left
        vertices[3] = x2; vertices[4] = y1; vertices[5] = z2;  // Bottom-right
        vertices[6] = x2; vertices[7] = y2; vertices[8] = z2;  // Top-right
        vertices[9] = x1; vertices[10] = y2; vertices[11] = z2; // Top-left
        ...
        // West face (X-)
        vertices[0] = x1; vertices[1] = y1; vertices[2] = z1;  // Bottom-left
        vertices[3] = x1; vertices[4] = y1; vertices[5] = z2;  // Bottom-right
        vertices[6] = x1; vertices[7] = y2; vertices[8] = z2;  // Top-right
        vertices[9] = x1; vertices[10] = y2; vertices[11] = z1; // Top-left
        ...
        // East face (X+)
        vertices[0] = x2; vertices[1] = y1; vertices[2] = z2;  // Bottom-left
        vertices[3] = x2; vertices[4] = y1; vertices[5] = z1;  // Bottom-right
        vertices[6] = x2; vertices[7] = y2; vertices[8] = z1;  // Top-right
        vertices[9] = x2; vertices[10] = y2; vertices[11] = z2; // Top-left
